...
void RenderSystem::render(Renderer* renderer)
{
    for (auto pair : m_render_map) {
        ...
        if (model->instanced())
            update_instanced_batch(renderer, model, batch);
...
void RenderSystem::update_instanced_batch(Renderer* renderer, Model* model, std::queue<TransformComponent*>& batch)
{
    ...
    renderer->update_instanced(model, models, num_entities);
    delete[] models;
}
...
{
    if (model->instanced()) {
        renderer->simple_render_instanced(model, model->num_instanced());
        return;
    }
...
{
    if (model->instanced()) {
        renderer->render_instanced_depth(depth_shader, model, batch.size());
        return;
    }
